...
For local integration, use backend base URL `http://localhost:8000`. Before an integration change, verify the deployed Swagger document at `http://localhost:8000/swagger/index.html` and `GET /health`. Whisper at `http://localhost:9000` is a backend-internal dependency; the browser does not call it directly.
